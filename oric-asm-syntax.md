...
The CPU hardware vectors (`SYS.NMIVEC` `$FFFA`, `SYS.RESETVEC` `$FFFC`, `SYS.IRQVEC` `$FFFE`) and the ROM cold-start entry (`SYS.RESET.V10` / `.V11`) are included.  The full 6522 VIA register file is included: ports (`SYS.ORB`, `SYS.ORA`, `SYS.ORANH`, `SYS.DDRB`, `SYS.DDRA`), Timer 1 (`SYS.T1CL`, `SYS.T1CH`, `SYS.T1LL`, `SYS.T1LH`), Timer 2 (`SYS.T2CL` ≡ `SYS.T2LL` both `$0308`, `SYS.T2CH`), and the misc control regs (`SYS.SR`, `SYS.ACR`, `SYS.PCR`, `SYS.IFR`, `SYS.IER`), plus a `SYS.VIA` block base.  `SYS.CHARSET` and `SYS.KEYCODETAB` are the ROM charset/keymap copies and are `.V10`/`.V11` variant (validated different per ROM).  The video-mode-variant areas are included: `SYS.SCREEN`, `SYS.STDCHARSET`, `SYS.ALTCHARSET` (each `.TEXT` / `.HIRES`).

Not yet included: the per-byte FAC zero-page cells; the page-2 NMI/IRQ "vector target" rows (overlap `NMIJP`/`INTFS`); the user-ML scratch area; "signed FAC→int".
