...
```asm
        ; Set TIMER3 to N (centiseconds): write LSB to $0276, MSB to $0277
        ; (write MSB last to avoid the IRQ catching a mid-update inconsistent value)
        LDA #lsb_of_N
        STA $0276
        LDA #msb_of_N
        STA $0277
...
wait_loop:
        LDA $0277
        ORA $0276
        BNE wait_loop      ; spin until TIMER3 decrements to zero
```
...
Call pattern from machine code (parameters set up beforehand):
...
```asm
        ...
        LDA #duration
        STA $02E3
        ; ... more params per routine ...
        JSR $FBD0       ; on V1.1b — patch operand to $FBB6 for V1.0
```
...
$0400:  JSR GTVALS         ; parse "X,Y" into screen coords ($D996 V1.0 / $DA22 V1.1b)
        ...
        INY                 ; (no-op equivalent on V1.1b since GTVALS already does this)
