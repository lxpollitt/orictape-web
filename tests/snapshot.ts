...
for (const filename of wavFiles) {
  ...
  for (let pi = 0; pi < programs.length; pi++) {
    ...
    if (prog.name && prog.lines.length > 0) {
      ...
      writtenTapFiles.push(tapFilename);
      totalTaps++;
    }
  }
...
// Warn about files in the output directory that weren't generated by this run.
const generatedFiles = new Set(writtenTapFiles);
generatedFiles.add('summary.txt');
const staleFiles = readdirSync(outputDir).filter(f => !generatedFiles.has(f));
if (staleFiles.length > 0) {
  console.warn(`\nWarning: ${staleFiles.length} file(s) in ${outputDir} were not generated by this run:`);
  for (const f of staleFiles.sort()) console.warn(`  ${f}`);
}
